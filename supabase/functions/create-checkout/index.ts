...
serve(async (req) => {
  ...
  try {
    // Missing auth is a client error (401), not a server crash. Without this
    // guard the `!` assertion + `.replace` below threw on a no-auth request
    // ("Cannot read properties of null") and surfaced as a 500.
