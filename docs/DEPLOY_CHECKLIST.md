...
- [ ] **`20260530140000_search_catalog.sql`** — adds pg_trgm + the `search_catalog(p_query, p_limit)` RPC powering the global search box (DB-backed, typo-tolerant, includes sealed products). **Run in SQL editor — no edge-function deploy needed.** Until it runs, the search box gracefully falls back to the old client-index search (cards only, no fuzzy). After it runs, search is instant + fuzzy + includes sealed automatically. Verify: `SELECT * FROM search_catalog('charzard', 5);` returns Charizard rows.
